...
export default class UniswapV2Relayer {
  ...
  getOrderExecutionParams(order: Order, handler: ethers.Contract, fee = ethers.BigNumber.from(1)): any {
    return [
      order.module,
      order.inputToken,
      order.owner,
      ...
      order.signature,
      this.base.abiCoder.encode(['address', 'address', 'uint256'], [handler.address, this.base.account.address, fee.toString()])
    ]
  }
}
